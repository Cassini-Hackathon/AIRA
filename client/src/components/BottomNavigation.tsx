...
const BottomNavigation = () => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-around py-2 px-4 z-40">
      <button 
        className={`flex flex-col items-center p-2 ${isActive('/') ? 'text-medical' : 'text-gray-500'}`}
        onClick={() => setLocation('/')}
      >
      ...
      <button 
        className={`flex flex-col items-center p-2 ${isActive('/guides') ? 'text-medical' : 'text-gray-500'}`}
      ...
      <button 
        className={`flex flex-col items-center p-2 ${isActive('/profile') ? 'text-medical' : 'text-gray-500'}`}
